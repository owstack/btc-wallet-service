...
function Explorer(opts) {
  $.checkArgument(opts);
  $.checkArgument(_.includes([Constants.LIVENET, Constants.TESTNET], opts.network));
  $.checkArgument(opts.url);

  this.apiPrefix = opts.apiPrefix || '/explorer-api';
  this.network = opts.network || Constants.LIVENET;
  this.hosts = opts.url;
  this.userAgent = opts.userAgent || 'btcws';
};
