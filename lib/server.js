...
/**
 * Gets the current version of BTCWS
 */
WalletService.getServiceVersion = function() {
  if (!serviceVersion)
    serviceVersion = 'btcws-' + require('../package').version;
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  if (!_.includes([Constants.LIVENET, Constants.TESTNET], opts.network))
  ...
  try {
    pubKey = new btcLib.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype.getWalletFromIdentifier = function(opts, cb) {
  ...
  async.parallel([
    ...
    async.detectSeries(_.values([Constants.LIVENET, Constants.TESTNET]), function(network, nextNetwork) {
      var bc = self._getBlockchainExplorer(network);
      if (!bc) return nextNetwork(false);
      bc.getTransaction(opts.identifier, function(err, tx) {
        if (err || !tx) return nextNetwork(false);
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  try {
    btcLib.HDPublicKey(opts.xPubKey);
  } catch (ex) {
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = btcLib.Address(addresses[0]).toObject().network;
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var btcTx = txp.getBtcTx();
    btcError = btcTx.getSerializationError(serializationOpts);
    if (!btcError) {
      txp.fee = btcTx.getFee();
    }
  } catch (ex) {
    log.error('Error building Btc transaction', ex);
    return ex;
  }

  if (btcError instanceof btcLib.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (btcError instanceof btcLib.errors.Transaction.DustOutputs)
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, btcLib.Transaction.DUST_AMOUNT);
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    var toAddress = {};
    try {
      toAddress = new btcLib.Address(output.toAddress);
    } catch (ex) {
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.copayerId, opts.signatures, copayer.xPubKey)) {
          ...
          var raw = txp.getBtcTx().uncheckedSerialize();
          log.warn('Raw tx:', raw);
...
WalletService._clearBlockchainHeightCache = function(network) {
  ...
  if (!_.includes([Constants.LIVENET, Constants.TESTNET], network)) {
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getNormalizedTxs(addresses, from, to, cb) {
    ...
    var network = btcLib.Address(addresses[0].address).toObject().network;
      ...
      async.waterfall([
        ...
        function(txs, next) {
          ...
          if (!_.isEmpty(txs.items)) {
            minTs = _.minBy(txs.items, 'time').time - 7 * 24 * 3600;
            maxTs = _.maxBy(txs.items, 'time').time + 1 * 24 * 3600;
          }
