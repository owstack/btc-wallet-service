...
/**
 * Creates an instance of the Btccore Wallet Service.
 * @constructor
 */
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new Btccore.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  try {
    Btccore.HDPublicKey(opts.xPubKey);
  } catch (ex) {
...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  var networkName = Btccore.Address(addresses[0]).toObject().network;
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var btccoreTx = txp.getBtccoreTx();
    btccoreError = btccoreTx.getSerializationError(serializationOpts);
    if (!btccoreError) {
      txp.fee = btccoreTx.getFee();
    }
  } catch (ex) {
    log.error('Error building Btccore transaction', ex);
    return ex;
  }

  if (btccoreError instanceof Btccore.errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (btccoreError instanceof Btccore.errors.Transaction.DustOutputs)
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, Btccore.Transaction.DUST_AMOUNT);
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    var toAddress = {};
    try {
      toAddress = new Btccore.Address(output.toAddress);
    } catch (ex) {
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.copayerId, opts.signatures, copayer.xPubKey)) {
          ...
          var raw = txp.getBtccoreTx().uncheckedSerialize();
          log.warn('Raw tx:', raw);
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function getNormalizedTxs(addresses, from, to, cb) {
    ...
    var network = Btccore.Address(addresses[0].address).toObject().network;
