...
Address.create = function(opts) {
  ...
  x.network = btcLib.Address(x.address).toObject().network;
...
Address._deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      btcAddress = btcLib.Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      btcAddress = btcLib.Address.fromPublicKey(publicKeys[0], network);
      break;
  }
