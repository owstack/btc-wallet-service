...
TxProposal.create = function(opts) {
  ...
  try {
    x.network = opts.network || Btccore.Address(x.outputs[0].toAddress).toObject().network;
  } catch (ex) {}
  $.checkState(_.contains(_.values([Constants.LIVENET, Constants.TESTNET]), x.network));
...
TxProposal.prototype._buildTx = function() {
  ...
  // Validate actual inputs vs outputs independently of Btccore
...
TxProposal.prototype.getBtccoreTx = function() {
  var self = this;
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToBtccoreTx(t, x.signatures, x.xpub);
  });
...
TxProposal.prototype._addSignaturesToBtccoreTx = function(tx, signatures, xpub) {
  var self = this;
...
TxProposal.prototype.sign = function(copayerId, signatures, xpub) {
  try {
    ...
    var tx = this.getBtccoreTx();
    this._addSignaturesToBtccoreTx(tx, signatures, xpub);
