...
TxProposal.create = function(opts) {
  ...
  try {
    x.network = opts.network || Btc.Address(x.outputs[0].toAddress).toObject().network;
  } catch (ex) {}
...
TxProposal.prototype._buildTx = function() {
  ...
  _.each(self.outputs, function(o) {
    ...
    if (o.script) {
      t.addOutput(new Btc.Transaction.Output({
        script: o.script,
        satoshis: o.amount
      }));
    } else {
...
TxProposal.prototype.getBtcTx = function() {
  var self = this;
...
TxProposal.prototype._addSignaturesToBtcTx = function(tx, signatures, xpub) {
  ...
  var i = 0,
    x = new Btc.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = Btc.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: Btc.crypto.Signature.SIGHASH_ALL,
        publicKey: pub,
      };
